...
def get_device(actual_args=None):
    ...
    logging.debug("now we will try to connect to the display")
    # create device
    try:
        device = cmdline.create_device(args)
    except error.Error as e:
        logging.debug("We caught an error and are going to parse it")
        return -1
#        parser.error(e)

    logging.debug("now we will return with the device")
    return device
